...
void Cutter::cutFromTime(Demo* demo, int time, int mapIndex){
	...
	//throw "gamestate instruction not found in message";
	...
	if (i == nextMapMessageindex)
		...
		//throw "starting time was not found";

	//std::cout << "beginning message index " << i << std::endl;
	...
	while (delta != 0){
		...
		if (!demo->getMessage(u) || /*K==5*/min_u + 1 == max_u){
		...
		if (snap && (snap->getDeltanum()>(j-i) ) ){
			...
			while (delta != 0){
				if (u<=0)
					...
					//throw "trying do delta from too old message";
