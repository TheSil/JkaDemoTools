...
void Demo::analyse(){
	...
	for(;messageId<count;++messageId){
		...
		//forceReload = false; //local variable
		...
		for(int i=0; i<msg->getInstructionsCount();++i){
			...
			if (instr->getType() == INSTR_SNAPSHOT){
				...
				following after this one, because it might be incorrect loaded
				*/
				...
				if (!awaitingMapChange){
					if ((lastSnapFlags != -1) &&
						((snap->getSnapflags() & 4) != lastSnapFlags)) {
							...
							int mapTime = impl->getStartTime(this,impl->maps.size()-1);
							impl->maps[impl->maps.size()-1].endTime = (lastSnapTime-mapTime)/1000;

							//insert
							impl->maps.push_back(DemoImpl::MapRef(messageId,"restart",true));

							//log starting time for current map
							//impl->maps[impl->maps.size()-1].startTime = 0;
					}
				}
				...
				if (minId == maxId){ //something went wrong, we didnt find seeking seq number
                    /*
					std::string s = "Corrupted demo format, seeking seqnumber could not be found ";
					s += seekingSeqNumber;
				...
				endIndex = s.find_first_of('\\',startIndex);
				...
				int mapTime;
				if (impl->maps.size() > 0){
					mapTime = impl->getStartTime(this,impl->maps.size()-1);
...
void Demo::unloadMessage(int id)
{
	...
	//std::cout << "DEBUG: Message[" << id << "] at " <<impl->messages[id].message << " deallocating"<< std::endl;
	delete impl->messages[id].message;
	impl->messages[id].message = 0;
}
...
int DemoImpl::getStartTime(Demo* demo, int mapIndex){
	...
	if (demo->isMapRestart(mapIndex)){
		...
		//throw "restart time extraction failed";
		...
		for(int i=0;i<firstMessage->getInstructionsCount();++i){
			...
			if (gamestate){
				...
				if (!(stream >> ret))
					...
					//throw "gamestate time extraction failed";
				return ret;
			}
		}
	}
